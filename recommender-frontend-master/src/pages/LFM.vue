<template>
  <v-app id="recommend">
    <v-container style="align-self: auto; text-align: center">
      <h1 class="my-16">Offline Recommendations</h1>
      <v-row>
        <v-col
          :key="index"
          align-self="center"
...
import { getWishMovies, getStreamMovie, rateToMovie } from '../api/movie';
...
export default {
  ...
  methods: {
    getRecs() {
      ...
      let count = 15;
      getWishMovies({ username: username, num: count })
        .then((res) => {
    ...
    getNewRecs(movie, newVal) {
      ...
      // })
      getStreamMovie(username, 5).then((rres) => {
